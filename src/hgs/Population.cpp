...
void Population::generatePopulation()
{
	...
	double fractionGeneratedSmart = params->config.fractionGeneratedSmart;
	...
	int nofSmartSolverToGenerate = round(fractionGeneratedSmart * nofIndividuals);
    ...
    for (int i = 0; i < nofSmartSolverToGenerate; i++)
    {
...
{
	INFO("----- RESET: CREATING A NEW POPULATION -----");
...
void Population::printState(int nbIter, int nbIterNoImprovement)
{
	...
	std::snprintf(a,500,"It %6d %6d | T(s) %.2f", nbIter, nbIterNoImprovement, params->getTimeElapsedSeconds());
	char b[500];
	...
	{
		std::snprintf(b,500," | Feas %zu %.2f %.2f", feasibleSubpopulation.size(), getBestFeasible()->myCostSol.penalizedCost, getAverageCost(feasibleSubpopulation));
	}
	else
	{
		std::snprintf(b,500," | NO-FEASIBLE");
	}
	char c[500];
	...
	{
		std::snprintf(c,500," | Inf %zu %.2f %.2f", infeasibleSubpopulation.size(), getBestInfeasible()->myCostSol.penalizedCost, getAverageCost(infeasibleSubpopulation));
	}
	else
	{
		std::snprintf(c,500," | NO-INFEASIBLE");
	}
	...
	std::snprintf(d,500," | Div %.2f %.2f", getDiversity(feasibleSubpopulation), getDiversity(infeasibleSubpopulation));
	std::snprintf(e,500," | Feas %.2f %.2f", static_cast<double>(std::count(listFeasibilityLoad.begin(), listFeasibilityLoad.end(), true)) / static_cast<double>(listFeasibilityLoad.size()), static_cast<double>(std::count(listFeasibilityTimeWarp.begin(), listFeasibilityTimeWarp.end(), true)) / static_cast<double>(listFeasibilityTimeWarp.size()));
	std::snprintf(f,500," | Pen %.2f %.2f", params->penaltyCapacity, params->penaltyTimeWarp);
...
Population::Population(Params* params, Split* split, LocalSearch* localSearch,hust::Solver* smartSolver)
: params(params), split(split), localSearch(localSearch),smartSolver(smartSolver)
{
